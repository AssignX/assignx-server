...
public class RoomExceptionUtils {
    ...
    public static RoomException RoomActionRequired() {
        return new RoomException(HttpStatus.BAD_REQUEST, "actionType은(는) 필수 입력값입니다.");
    }

    public static RoomException RoomReqDTOValidation(String missingValues) {
        String message = missingValues + " 은(는) 필수 입력값입니다.";
        return new RoomException(HttpStatus.BAD_REQUEST, message);
    }
